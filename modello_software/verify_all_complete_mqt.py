...
import math
import cmath
from utiles import testVLSI, testQiskit, testQiskitQASM, KLD, HeligerFidelity, MaximumComplexDistance, AverageComplexDistance
...
#Floating point
pathEmulatorIstructions = "../../file/compiled_mqt_dec/"
pathOutputEmulator = "../../file/simulated_mqt/"
pathOutputQiskit = "../../file/qiskit_res_mqt/"
pathOutputQiskitQASM = "../../file/qiskit_qasm_res_mqt/"
path = "../../file/MQTBench_input/"
files = os.listdir(path)
for file in files:
    print("\n\execution of " + file +":\n\n")
    ...
    if qubit <= 16:
        ...
        if done:
            if Lenght in EmulatorTimeForLenght_float.keys():
                ...
                RepetitionsForLenght_float[Lenght] += 1
            else: 
                EmulatorTimeForLenght_float[Lenght] = timeVLSI
                RepetitionsForLenght_float[Lenght] = 1
            ...
            if done == True:
                ...
                if len(probabilityDensity) < 33:
                    ...
                    plt.savefig("../../file/probabilityBarPlot_mqt/" + circuit + ".eps", format='eps')
                    plt.savefig("../../file/probabilityBarPlot_mqt/" + circuit + ".png", format='png')
                    plt.savefig("../../file/probabilityBarPlot_mqt/" + circuit + ".pdf", format='pdf')
                    plt.close()
            else:
...
for key in EmulatorTimeForLenght_float.keys():
    ...
    QiskitTimeForLenght[key] = QiskitTimeForLenght[key]/RepetitionForLenght_float[key]
    QiskitQASMTimeForLenght[key] = QiskitQASMTimeForLenght[key]/RepetitionForLenght_float[key]
    kld_res_dict_float[key] = kld_res_dict_float[key]/RepetitionForLenght_float[key]
    HF_res_dict_float[key] = HF_res_dict_float[key]/RepetitionForLenght_float[key]
    MCD_dict_float[key] = MCD_dict_float[key]/RepetitionForLenght_float[key]
    ACD_dict_float[key] = ACD_dict_float[key]/RepetitionForLenght_float[key]
    
DataFramePandas = pd.DataFrame.from_dict(EmulatorTimeForLenght_float[approximation_mechanism])
DataFramePandas.to_csv("Time_mqt.csv")
DataFramePandas = pd.DataFrame.from_dict(QiskitTimeForLenght[approximation_mechanism])
DataFramePandas.to_csv("Time_mqt_qiskit_state_vector.csv")
DataFramePandas = pd.DataFrame.from_dict(QiskitQASMTimeForLenght[approximation_mechanism])
DataFramePandas.to_csv("Time_mqt_qiskit_qasm_simulator.csv")
DataFramePandas = pd.DataFrame.from_dict(kld_res_dict_float[approximation_mechanism])
DataFramePandas.to_csv("kld_mqt.csv")
DataFramePandas = pd.DataFrame.from_dict(HF_res_dict_float[approximation_mechanism])
DataFramePandas.to_csv("HF_mqt.csv")
DataFramePandas = pd.DataFrame.from_dict(MCD_dict_float[approximation_mechanism])
DataFramePandas.to_csv("MCD_mqt.csv")
DataFramePandas = pd.DataFrame.from_dict(ACD_dict_float[approximation_mechanism])
...
leg = plt.legend(loc='upper right', frameon=True, fontsize=15)
leg.get_frame().set_facecolor('white')
plt.savefig("EmulatorTime_mqt.eps", format='eps')
plt.savefig("EmulatorTime_mqt.png", format='png')
plt.savefig("EmulatorTime_mqt.pdf", format='pdf')
plt.close()
...
leg = plt.legend(loc='upper right', frameon=True, fontsize=15)
leg.get_frame().set_facecolor('white')
plt.savefig("QiskitTime_mqt.eps", format='eps')
plt.savefig("QiskitTime_mqt.png", format='png')
plt.savefig("QiskitTime_mqt.pdf", format='pdf')
plt.close()
...
leg = plt.legend(loc='upper right', frameon=True, fontsize=15)
leg.get_frame().set_facecolor('white')
plt.savefig("QiskitQASMTime_mqt.eps", format='eps')
plt.savefig("QiskitQASMTime_mqt.png", format='png')
plt.savefig("QiskitQASMTime_mqt.pdf", format='pdf')
plt.close()
...
leg = plt.legend(loc='upper right', frameon=True, fontsize=15)
leg.get_frame().set_facecolor('white')
plt.savefig("ComparisonTime_mqt.eps", format='eps')
plt.savefig("ComparisonTime_mqt.png", format='png')
plt.savefig("ComparisonTime_mqt.pdf", format='pdf')
plt.close()
...
for approximation_mechanism in approximation_mechanisms:
    ...
    for parallelism in parallelisms: 
        ...
        pathOutputEmulator = "../file/simulated_mqt_" + approximation_mechanisms_acronym[approximation_mechanism] + "_" + format(parallelism)  + "/"
    
        if not os.path.exists(pathOutputEmulator):
            os.makedirs(pathOutputEmulator)
            
        pathOutputQiskit = "../file/qiskit_res_mqt_" + approximation_mechanisms_acronym[approximation_mechanism] + "_" + format(parallelism)  + "/"
    
        if not os.path.exists(pathOutputQiskit):
            os.makedirs(pathOutputQiskit)
        
        pathEmulatorIstructions = "../../file/compiled_mqt_dec_" + approximation_mechanisms_acronym[approximation_mechanism] + "_" + format(parallelism)  + "/"
            ...
            if qubit <= 16:
                ...
                if done:
                    if Lenght in EmulatorTimeForLenght[approximation_mechanism][parallelism].keys():
                        ...
                        RepetitionsForLenghtEmulator[approximation_mechanism][parallelism][Lenght] += 1
                    else: 
                        EmulatorTimeForLenght[approximation_mechanism][parallelism][Lenght] = timeVLSI
                        RepetitionsForLenghtEmulator[approximation_mechanism][parallelism][Lenght] = 1
                    ...
                    if len(probabilityDensity) < 33:
                        ...
                        leg = plt.legend(loc='upper right', frameon=True, fontsize=15)
                        leg.get_frame().set_facecolor('white')
                        plt.savefig("../../file/probabilityBarPlot_mqt/" + circuit + "_" + approximation_mechanism + "_" +parallelism + ".eps", format='eps')
                        plt.savefig("../../file/probabilityBarPlot_mqt/" + circuit + "_" + approximation_mechanism + "_" +parallelism +".png", format='png')
                        plt.savefig("../../file/probabilityBarPlot_mqt/" + circuit + "_" + approximation_mechanism + "_" +parallelism +".pdf", format='pdf')
                        plt.close()
                else:
                    print("My solver cannot simulate the file\n")   
            else:
    ...
    DataFramePandas.to_csv("Time_mqt_"+ approximation_mechanism +".csv")
    DataFramePandas = pd.DataFrame.from_dict(kld_res_dict[approximation_mechanism])
    DataFramePandas.to_csv("kld_mqt_"+ approximation_mechanism +".csv")
    DataFramePandas = pd.DataFrame.from_dict(HF_res_dict[approximation_mechanism])
    DataFramePandas.to_csv("HF_mqt_"+ approximation_mechanism +".csv")
    DataFramePandas = pd.DataFrame.from_dict(MCD_dict[approximation_mechanism])
    DataFramePandas.to_csv("MCD_mqt_"+ approximation_mechanism +".csv")
    DataFramePandas = pd.DataFrame.from_dict(ACD_dict[approximation_mechanism])
    DataFramePandas.to_csv("ACD_mqt_"+ approximation_mechanism +".csv")
    i = 0 
    for parallelism in parallelisms:
    ...
    plt.savefig("EmulatorTime_mqt_" + approximation_mechanism + ".eps", format='eps')
    plt.savefig("EmulatorTime_mqt_" + approximation_mechanism + ".png", format='png')
    plt.savefig("EmulatorTime_mqt_" + approximation_mechanism + ".pdf",, format='pdf')
    plt.close() 
    ...
    leg.get_frame().set_facecolor('white')
    plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    plt.savefig("HF_mqt_" + approximation_mechanism + ".eps", format='eps')
    plt.savefig("HF_mqt_" + approximation_mechanism + ".png", format='png')
    plt.savefig("HF_mqt_" + approximation_mechanism + ".pdf",, format='pdf')
    plt.close()    
    ...
    leg.get_frame().set_facecolor('white')
    plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    plt.savefig("Kld_mqt_" + approximation_mechanism + ".eps", format='eps')
    plt.savefig("Kld_mqt_" + approximation_mechanism + ".png", format='png')
    plt.savefig("Kld_mqt_" + approximation_mechanism + ".pdf",, format='pdf')
    plt.close() 
    ...
    leg.get_frame().set_facecolor('white')
    plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    plt.savefig("MCD_mqt_" + approximation_mechanism + ".eps", format='eps')
    plt.savefig("MCD_mqt_" + approximation_mechanism + ".png", format='png')
    plt.savefig("MCD_mqt_" + approximation_mechanism + ".pdf",, format='pdf')
    plt.close()  
    ...
    leg.get_frame().set_facecolor('white')
    plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    plt.savefig("ACD_mqt_" + approximation_mechanism + ".eps", format='eps')
    plt.savefig("ACD_mqt_" + approximation_mechanism + ".png", format='png')
    plt.savefig("ACD_mqt_" + approximation_mechanism + ".pdf",, format='pdf')
    plt.close()                   

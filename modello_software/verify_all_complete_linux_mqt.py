...
fileErrorsname = "errors_file_mqt.txt"
try:
...
for approximation_mechanism in approximation_mechanisms:
    ...
    for parallelism in parallelisms: 
        ...
        pathOutputEmulator = "../../file/simulated_mqt" + approximation_mechanisms_acronym[approximation_mechanism] + "_" + format(parallelism)  + "/"
                ...
                if done:
                    ...
                    if doneQiskit: 
                        ...
                        if len(probabilityDensity) < 33:
                            ...
                            leg = plt.legend(loc='upper right', frameon=True, fontsize=15)
                            leg.get_frame().set_facecolor('white')
                            plt.savefig("../../file/probabilityBarPlot_mqt/" + circuit + "_" + approximation_mechanism + "_" +format(parallelism) + ".eps", format='eps')
                            plt.savefig("../../file/probabilityBarPlot_mqt/" + circuit + "_" + approximation_mechanism + "_" +format(parallelism) +".png", format='png')
                            plt.savefig("../../file/probabilityBarPlot_mqt/" + circuit + "_" + approximation_mechanism + "_" +format(parallelism) +".pdf", format='pdf')
                            plt.close()                       
...
for approximation_mechanism in approximation_mechanisms:
    ...
    DataFramePandas.to_csv("MSD_mqt_"+ approximation_mechanism +".csv")
    DataFramePandas = pd.DataFrame.from_dict(MinSD_dict[approximation_mechanism])
    DataFramePandas.to_csv("MinSD_mqt_"+ approximation_mechanism +".csv")
    DataFramePandas = pd.DataFrame.from_dict(ASD_dict[approximation_mechanism])
    DataFramePandas.to_csv("ASD_mqt_"+ approximation_mechanism +".csv")

...
for approximation_mechanism in approximation_mechanisms:
    ...
    for parallelism in parallelisms: 
        ...
        pathOutputQiskit = "../file/qiskit_res_" + approximation_mechanisms_acronym[approximation_mechanism] + "_" + format(parallelism)  + "/"
            ...
            if qubit <= 16:
                ...
                if done:
                    ...
                    if len(probabilityDensity) < 33:
                        ...
                        leg = plt.legend(loc='upper right', frameon=True, fontsize=15)
                        leg.get_frame().set_facecolor('white')
                        plt.savefig("../../file/probabilityBarPlot/" + circuit + "_" + approximation_mechanism + "_" +parallelism + ".eps", format='eps')
                        plt.savefig("../../file/probabilityBarPlot/" + circuit + "_" + approximation_mechanism + "_" +parallelism +".png", format='png')
                        plt.savefig("../../file/probabilityBarPlot/" + circuit + "_" + approximation_mechanism + "_" +parallelism +".pdf", format='pdf')
                        plt.close()
                else:
                    print("My solver cannot simulate the file\n")   
            else:
    ...
    plt.fill_between(x,0.95, 1, alpha=0.2, color='r')
    plt.xlim([0, 1])    
    ...
    for parallelism in parallelisms:
        ...
        i += 1
    plt.axhline(y = 0.05, color = 'r', linewidth=3,  linestyle = '--')  
    plt.fill_between(x,0, 0.05, alpha=0.2, color='r')
    plt.xlim([0, 1])     

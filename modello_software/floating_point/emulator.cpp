...
//They work with floting point data representation
#include "emulator.h"
#include <iostream>
#include <fstream>
#include <string>
...
void execute_H (qstate &x, qstate &y){
    ...
    x.real = (float) INV_SQRT_2 * ( tmp_r + y.real );
    x.img = (float) INV_SQRT_2 * ( tmp_i + y.img );

    y.real = (float) INV_SQRT_2 * ( tmp_r - y.real );
    y.img =  (float) INV_SQRT_2 * ( tmp_i - y.img );    
...
void execute_T (qstate &y){
    ...
    y.real = (float) INV_SQRT_2 * ( tmp - y.img );
    y.img = (float) INV_SQRT_2 * ( tmp + y.img );
...
void execute_TDG (qstate &y){
    ...
    y.real = (float) INV_SQRT_2 * ( tmp + y.img );
    y.img = (float) INV_SQRT_2 * ( y.img - tmp );
...
void execute_RX (qstate &x, qstate &y, gon &imm){
    ...
    x.real  = tmp_r * imm.cos + y.img * imm.sin;
    x.img = tmp_i * imm.cos - y.real * imm.sin;

    y.real = tmp_i * imm.sin + y.real * imm.cos;
    y.img = y.img * imm.cos - tmp_r * imm.sin;
...
void execute_RY (qstate &x, qstate &y, gon &imm){
    ...
    x.real = tmp_r * imm.cos - y.real * imm.sin;
    x.img = tmp_i * imm.cos - y.img * imm.sin;

    y.real = tmp_r * imm.sin + y.real * imm.cos;
    y.img = tmp_i * imm.sin + y.img * imm.cos;
}
...
void execute_RZ (qstate &x, qstate &y, gon &imm){
    ...
    x.real = tmp * imm.cos + x.img * imm.sin;
    x.img = x.img * imm.cos - tmp * imm.sin;

    tmp = y.real;

    y.real = tmp * imm.cos - y.img * imm.sin;
    y.img = tmp * imm.sin + y.img * imm.cos;
...
int emulatorVLSI(string istructionfilename, string sinecosinefilename, string outputfilename, int parallelism, bool verbose){
	...
	if ( sincosNumber != 0 ) {
		...
		while ( sincosFile.getline(lines, 50)){
			...
			if (sscanf(lines, "%d %d\n",  &val_sin, &val_cos) == 2){
				...
				sincosVector[sincosIndex].sin = (float) (double(val_sin)/pow(2.0,(double)(parallelism - 2)));

				//convert the value in floating point and put it inside the vector
				sincosVector[sincosIndex].cos = (float) (double(val_cos)/pow(2.0,(double)(parallelism - 2)));
	...
	if (verbose == true){
		...
		for (unsigned int i=0; i < stateNumber; i ++){
			...
			fprintf(stdout, "%d: %f +j%f\n",i,stateVector[i].real,stateVector[i].img);

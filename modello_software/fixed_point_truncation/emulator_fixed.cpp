...
//They work with fixed point data representation considering truncation rounding technique
#include "emulator_fixed.h"
#include <iostream>
#include <fstream>
#include <string>
#include <math.h>
...
void execute_H (qstate &x, qstate &y){
    ...
    x.real =  (INV_SQRT_2 * ( tmp_r + y.real )) >> frac;
    x.img =  (INV_SQRT_2 * ( tmp_i + y.img )) >> frac;

    y.real =  (INV_SQRT_2 * ( tmp_r - y.real )) >> frac;
    y.img = (INV_SQRT_2 * ( tmp_i - y.img )) >> frac;    
...
void execute_T (qstate &y){
    ...
    y.real = (INV_SQRT_2 * ( tmp - y.img )) >> frac;
    y.img = (INV_SQRT_2 * ( tmp + y.img )) >> frac;
...
void execute_TDG (qstate &y){
    ...
    y.real = (INV_SQRT_2 * ( tmp + y.img )) >> frac;
    y.img = (INV_SQRT_2 * ( y.img - tmp )) >> frac;
...
void execute_RX (qstate &x, qstate &y, gon &imm){
    ...
    x.real  = ((tmp_r * imm.cos) >> frac) +((y.img * imm.sin) >> frac);
    x.img = ((tmp_i * imm.cos) >> frac) - ((y.real * imm.sin) >> frac);

    y.real = ((tmp_i * imm.sin) >> frac) + ((y.real * imm.cos)>> frac);
    y.img = ((y.img * imm.cos)>> frac) - ((tmp_r * imm.sin)>> frac);
...
void execute_RY (qstate &x, qstate &y, gon &imm){
    ...
    x.real = ((tmp_r * imm.cos)>> frac) - ((y.real * imm.sin)>> frac);
    x.img = ((tmp_i * imm.cos)>>frac) - ((y.img * imm.sin)>>frac);

    y.real = ((tmp_r * imm.sin)>> frac) + ((y.real * imm.cos)>> frac);
    y.img = ((tmp_i * imm.sin)>> frac) + ((y.img * imm.cos)>> frac);
}
...
void execute_RZ (qstate &x, qstate &y, gon &imm){
    ...
    x.real = ((tmp * imm.cos)>>frac) + ((x.img * imm.sin)>>frac);
    x.img = ((x.img * imm.cos)>>frac) - ((tmp * imm.sin)>>frac);

    tmp = y.real;

    y.real = ((tmp * imm.cos)>>frac) - ((y.img * imm.sin)>>frac);
    y.img = ((tmp * imm.sin)>>frac) + ((y.img * imm.cos)>> frac);
...
int emulatorVLSI_fixed(string istructionfilename, string sinecosinefilename, string outputfilename, int parallelism, bool verbose){
	...
	INV_SQRT_2 = (long int)(0.70710678118654752440084436210485*pow(2.0, (double)frac));
	...
	stateVector[0].real= (long int)1*((long int)pow(2.0, (double) frac));
	stateVector[0].img=0;
